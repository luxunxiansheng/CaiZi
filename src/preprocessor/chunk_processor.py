    ...
    def __call__(
        ...
        if len(token_ids) >= self.block_size:
            for i in range(0, len(token_ids) - self.block_size, self.stride):
                ...
                outputs.append({"input_ids": input_chunk, "target_ids": target_chunk})

        else:
            input_chunk = token_ids[: self.block_size]
            target_chunk = token_ids[1 : self.block_size + 1]
            outputs.append({"input_ids": input_chunk, "target_ids": target_chunk})
